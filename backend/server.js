...
// In-memory OTP store: { key -> { otp, expires, username } }
...
app.post('/api/auth/send-otp', authLimiter, async (req, res) => {
    const { type, value } = req.body; // type: 'email' | 'phone', value: email or phone
        ...
        if (emailConfigured) {
            try {
                await emailTransporter.sendMail({
                    ...
                    to: value,
                    subject: 'Your Nexus Chat OTP Code',
                    html: `
                        <div style="font-family:sans-serif;max-width:400px;margin:0 auto;background:#1a1a2e;padding:30px;border-radius:12px;color:#fff">
                            <h2 style="color:#6c63ff;margin-top:0">Nexus Chat</h2>
                            <p>Your one-time login code is:</p>
                            <div style="font-size:36px;font-weight:700;letter-spacing:12px;color:#fff;background:#0f0f1a;padding:20px;border-radius:8px;text-align:center">${otp}</div>
                ...
                // Fall through — still log OTP to console as fallback
            }
        }
        // Always log to console for dev/testing
        console.log(`\n==============================`);
        console.log(`[OTP] Email: ${value}`);
        console.log(`[OTP] Code:  ${otp}`);
        console.log(`==============================\n`);
    } else if (type === 'phone') {
        // Phone OTP: console mock (plug in Twilio later)
        console.log(`\n==============================`);
        console.log(`[OTP] Phone: ${value}`);
        console.log(`[OTP] Code:  ${otp}`);
        console.log(`==============================\n`);
    }
...
app.post('/api/auth/verify-otp', authLimiter, async (req, res) => {
    const { type, value, otp, username } = req.body;
    ...
    if (stored.otp !== otp.trim()) return res.status(400).json({ error: 'Invalid OTP code.' });

    otpStore.delete(key);

    // Find or create user based on email/phone
    const fieldCol = type === 'email' ? 'email' : 'phone';
    const displayName = username?.trim() || value.split('@')[0].replace(/[^a-zA-Z0-9_]/g, '').substring(0, 20) || 'user';

    try {
        let user;
        const { rows } = await db.query(`SELECT * FROM users WHERE ${fieldCol} = $1`, [value.toLowerCase().trim()]);
        if (rows.length > 0) {
            user = rows[0];
        } else {
            // Create new user
            ...
            const { rows: existing } = await db.query(`SELECT id FROM users WHERE username = $1`, [finalUsername]);
            if (existing.length > 0) finalUsername = finalUsername + '_' + Math.floor(Math.random() * 9000 + 1000);

            const dummyHash = await bcrypt.hash(Math.random().toString(36), 10);
            const { rows: newRows } = await db.query(
                `INSERT INTO users (username, password, ${fieldCol}) VALUES ($1, $2, $3) RETURNING *`,
                [finalUsername, dummyHash, value.toLowerCase().trim()]
            );
            user = newRows[0];
            io.emit('user_list_updated');
        }
...
app.post('/api/auth/google', authLimiter, async (req, res) => {
    ...
    if (!googleId || !email) return res.status(400).json({ error: 'Google auth data missing' });

    try {
        // Find by google_id first, then by email
        let user;
        const { rows: byGoogle } = await db.query(`SELECT * FROM users WHERE google_id = $1`, [googleId]);
        if (byGoogle.length > 0) {
            user = byGoogle[0];
        } else {
            const { rows: byEmail } = await db.query(`SELECT * FROM users WHERE email = $1`, [email.toLowerCase()]);
            if (byEmail.length > 0) {
                // Link google_id to existing account
                await db.query(`UPDATE users SET google_id = $1 WHERE id = $2`, [googleId, byEmail[0].id]);
                user = byEmail[0];
            } else {
                // Create new user from Google
                let finalUsername = (name || email.split('@')[0]).replace(/[^a-zA-Z0-9_]/g, '').substring(0, 20);
                const { rows: existing } = await db.query(`SELECT id FROM users WHERE username = $1`, [finalUsername]);
                if (existing.length > 0) finalUsername = finalUsername + '_' + Math.floor(Math.random() * 9000 + 1000);

                const dummyHash = await bcrypt.hash(Math.random().toString(36), 10);
                const { rows: newRows } = await db.query(
                    `INSERT INTO users (username, password, email, google_id) VALUES ($1, $2, $3, $4) RETURNING *`,
                    [finalUsername, dummyHash, email.toLowerCase(), googleId]
                );
                user = newRows[0];
                io.emit('user_list_updated');
            }
