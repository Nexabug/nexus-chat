...
function VoiceMessagePlayer({ url }) {
  ...
  return (
    <div className="voice-player">
      ...
      <span className="voice-time-label">{fmt(currentTime)} / {fmt(duration || 0)}</span>
    </div>
  );
}
                      ...
                      notifications.map(n => (
                        <div key={n.id} className="notification-item" onClick={() => { joinRoom(socket, n.room); setShowNotifications(false); }}>
                          <div className="notification-item-header">
                            ...
                            <span className="notification-time">{new Date(n.time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                          </div>
